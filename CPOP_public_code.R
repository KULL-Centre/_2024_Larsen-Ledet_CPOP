...
#Box plots for secondary structure elements for insertions
...
ggplot(dssp_cpop_ins, aes(experiment, score, fill = sec), na.rm = F) +
  ...
  geom_boxplot() +
  stat_summary(fun.data = n_fun, aes(group = sec), geom = "text", position = position_dodge(0.75)) +
  labs(y = "CPOP score", x = "") +
...
#Box plots for secondary structure elements for deletions
...
ggplot() +
  ...
  scale_colour_gradient2(limits = c(-0.2293587, 2.2293587), 
                         ...
                         mid = 'white', 
                         high = 'darkblue', 
                         labels = c("Low stability", "WT stability", "High stability")) +
